...
//		이하는 모르겠음
	...
	public void ex6() {
		// 점심 메뉴 뽑기 프로그램 (랜덤, 배열 길이는 자유)
		// 오늘 점심 메뉴 : 파스타
		
		
		String[] arr = {"김밥", "샌드위치", "햄버거", "백반", "국밥", "파스타"};
		...
		// 0 <= (int)(x*6) < 6
		// -> 0 1 2 3 4 5
		// ( Math.random*6 ) 값에 소괄호 하지 않고 (int) 붙이면 
		//맨처음값에 (int)가 붙고나서 *6이 되므로 0만 나옴에 주의
	...
	public void ex7() {
		...
		boolean flag = false; // 검색 전에는 없다고 가정.
	...
	public void ex8() {
		...
		for(int i=0; i<arr.length; i++) { 
			...
			if(arr[i].equals(input)) { //유의할 점: String 비교시 equals()사용
